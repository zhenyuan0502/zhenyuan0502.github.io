function scrollSpy(menuSelector, options) {
  ...
  if (menu) {
    var offset = (options = options || {}).offset || 0,
      ...
      scollTarget = $(".content :header").find("a.headerlink"),
      lastId = null,
      active = $();
    $(window).scroll(function () {
      ...
      if (lastId !== id) {
        ...
        for (
          var newActive = [], target = menu.find("[href='#" + id + "']");
          target.length && !target.is(menu);
          target = target.parent()
        )
          target.is("li") && newActive.push(target[0]);
...
var Home = location.href,
  ...
  Obsidian = {
      ...
      (xhrUrl = url),
        ...
        (xhr = $.ajax({
          ...
          error: function (a, b, c) {
            "abort" == b ? err && err() : (window.location.href = url),
              (xhrUrl = "");
          },
        }));
    },
              ...
              ? ($("#preview").css("position", "fixed"),
                setTimeout(function () {
                  $("#preview").removeClass("show"),
                    ...
                    window.scrollTo(
                      0,
                      parseInt($("#container").data("scroll"))
                    ),
                    setTimeout(function () {
    ...
    HS: function (tag, flag) {
      ...
      ($("#preview").length && window.history && history.pushState) ||
        (location.href = url),
        Obsidian.loading();
        ...
        if ($(data).filter("#single").length) {
          ...
          setTimeout(function () {
            Obsidian.player(),
              ...
              1 == comment.data("ae") && comment.click();
          }, 0);
    ...
    player: function () {
      ...
      p.length
        ? ("" == $("#audio source").eq(0).attr("src") &&
            ...
            (mp3 = audiolist.eq([
              Math.floor(Math.random() * audiolist.size()),
            ])),
            (p[0].src = mp3.data("url"))),
          1 == p.eq(0).data("autoplay") && p[0].play(),
          p.on({
            timeupdate: function () {
              var progress = (p[0].currentTime / p[0].duration) * 100;
              $(".bar").css("width", progress + "%"),
                (p[0].volume = progress / 5 <= 1 ? progress / 5 : 1);
            },
            ended: function () {
    ...
    setCodeRowWithLang: function () {
      ...
      code &&
        ...
        code.each(function () {
          ...
          (displayLangText =
            (lang =
              item[0].className.indexOf(" ") > -1
                ? item[0].className.split(" ")[0]
                : item[0].className) in langMap
              ? langMap[lang]
              : lang),
            item.find(".language-mark").length <= 0 &&
              item.prepend(
        ...
        $(".toc-item").on("scrollspy", function () {
          ...
          $(this).height() == link.height() &&
            (thisTop <= 0
              ...
              : tocHeight <= thisTop &&
                toc.scrollTop(
                  tocTop + thisTop + link.outerHeight() - tocHeight
                ));
        });
    },
  };
$(function () {
  Obsidian.P() && $("body").addClass("touch"),
    $("#preview").length
      ? (Obsidian.PS(),
      ...
      if (
        ...
        switch (!0) {
          ...
          case "prev" == rel || "next" == rel:
            if ("prev" == rel) var t = $("#prev_next a")[0].text;
            else var t = $("#prev_next a")[1].text;
            return (
              $(e.target).attr("title", t),
              Obsidian.HS($(e.target), "replace"),
              !1
            );
          case -1 != tag.indexOf("toc-text") ||
            -1 != tag.indexOf("toc-link") ||
            -1 != tag.indexOf("toc-number"):
            return (
              (hash = ""),
              ...
              $("html,body").animate({ scrollTop: to.offset().top - 80 }, 300),
              !1
            );
    ...
    (comment = $("#gitalk-container")),
    1 == comment.data("ae") && comment.click(),
    $(".article").length && (Obsidian.tocSpy(200), buildImgCaption()),
